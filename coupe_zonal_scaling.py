...
list_flux=["COS-ANTHR-Zumkehr-Month","COS-BB-UU-Month","COS-SOIL-SIB4-Month","COS-VEG-SIB4-Month","COS-OCEAN-Lennartz-Month"]
...
if reference=="average":
  ...
  for imm,model in enumerate(list_models):
   for iff,flux in enumerate(list_flux):
    name_file=dir_plot+'/'+model+"/TIER4/COS_"+flux+"_"+model+"_TIER4.nc23"
    if model == "TM5_UU": name_file=dir_plot+'/'+model+"/TIER4/COS_"+flux+"_"+model+"_TIER4.nc2"
    tmp=xr.open_dataset(name_file)
...
ax[0,0].set_ylabel("Pa (hPa)",labelpad=0.05)
...
cb_ax = f.add_axes([xmax+(xmax-xmin)*0.05, ymin, 0.015, (ymax-ymin)])
cbar=f.colorbar(l2, ax=ax[0,0], cax=cb_ax)
cbar.set_label('COS [ppt]')
if season=="sum":
 label=np.arange(450,750,30).tolist()
 for ii in range(0,len(label),2):
  label[ii]=" "
 cbar.set_ticklabels(label)
cbar.ax.tick_params(color='k')


level_antd=np.arange(-20,21,4)
row=1;col=0
list_models=[mm for mm in list_models if mm not in reference]
for imm,mm in enumerate(list_models):
  for iff,flux in enumerate(list_flux):
    name_file=dir_plot+'/'+mm+"/TIER4/COS_"+flux+"_"+mm+"_TIER4.nc23"
    if mm == "TM5_UU": name_file=dir_plot+'/'+mm+"/TIER4/COS_"+flux+"_"+mm+"_TIER4.nc2"
    tmp=xr.open_dataset(name_file)
  ...
  ax[row,int(col)].set_title(name_models[imm]+" - Ref",pad=0.05,fontsize=11)
  ...
  if col==0: ax[row,int(col)].set_ylabel("Pa (hPa)",labelpad=0.02)
  ...
  if (row==2)&(col==1): 
   ...
   cbar_ax = f.add_axes([xmax-(xmax-xmin)*0.05, ymin, 0.02,  (ymax-ymin)])
...
f.subplots_adjust(wspace=0.07,hspace=0.25,left=0.1,right=0.87)

ax[0,1].set_axis_off()
ax[0,2].set_axis_off()
#f.suptitle(suptitre)
plt.savefig("FIG/"+suptitre+"2")
plt.show()

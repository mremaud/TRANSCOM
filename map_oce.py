...
list_model=["SIB4","ORC"]
dirflux='/home/surface1/mremaud/COS/INPUT_sflx/TRANSCOM_FLUX/COS_Flux_1x1/COS_Ocean_1x1/'
nomvar="flux"
titre=['a) SIB4','b) ORC-SIB4','c) |ORC-SIB4|/SIB4']

for yy in range(2011,2019):
   dataset = xr.open_dataset(dirflux+"/COS-OCEAN-Lennartz-Month_"+str(yy)+".nc")
...
plt.savefig("FIG/OCE.png",format="png")


for yy in range(2011,2019):
   dataset = xr.open_dataset(dirflux+"/COS-OCEAN-LennartzPICSES-Month_"+str(yy)+".nc")
   longitude1=dataset.lon.values
   latitude1=dataset.lat.values
   if yy == 2011:
...
plt.title("b) DMS (PISCES)-DMS (Lennartz)")
ax=m.contourf(Y,X,(PIC-REF)*10**7,v,cmap=plt.cm.PiYG_r,extend='both')
cb=plt.colorbar(ax,orientation='horizontal',extend='both')
cb.set_label('$10^7$ mmol/m2/year')
plt.savefig("FIG/DMS-diff.png",format="png")
...
cb=plt.colorbar(ax,orientation='horizontal',extend='both')
plt.savefig("FIG/diff-DMS_perc.png",format="png")

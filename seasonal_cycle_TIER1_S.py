...
nmodels=["OBS","TOMCAT"      ,"LMDZ6"    ,"TM5","TM3","MIROC4","NICAM5","NICAM6"]
#models=["LMDZ3_LSCE"]
#nmodels=["LMDZ3"]
...
row=0
col=0
for station in ['ALT',"KUM",'BRW','SUM','MHD','LEF','MLO','CGO','HFM','THD','GIF','NWR','SMO','PSA','SPO']:
 for im,mm in enumerate(models):
  ...
  if (not data2.empty)&(mm!="LMDZ3_LSCE"): ax[row,col].plot(data2.index,data2.meas,color=colors[im],label=nmodels[im])
 if col==0: ax[row,col].set_ylabel("COS [ppt]",fontsize=14) 
 ax[row,col].set_xlim(1,12)
 ax[row,col].tick_params(rotation=60,labelsize=6,axis="x") 
 ax[row,col].set_title(station)
 if (row==4)&(col==0): ax[row,col].legend(fontsize=12,bbox_to_anchor=(3.1, -0.3),ncol=4)
# ax[row].set_xlabel("Month",fontsize=14)
 ax[row,col].tick_params(direction='in', length=3, color='k',grid_color="grey",colors="grey",
...
plt.savefig('/home/users/mremaud/PYTHON/COS/TRANSCOM/FIG/TIER1_SC_S.png',format='png', bbox_inches='tight', dpi = 200)
